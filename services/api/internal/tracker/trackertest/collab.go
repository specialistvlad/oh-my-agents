...
func runComments(t *testing.T, newStore Factory) {
	...
	t.Run("edits and deletes with a version", func(t *testing.T) {
		...
		edited, err := s.EditComment(ctx, c.ID, c.Version, "fixed")
		if err != nil {
		...
		if _, err := s.EditComment(ctx, c.ID, c.Version, "again"); !errors.Is(err, tracker.ErrVersionConflict) {
			t.Errorf("stale edit = %v, want ErrVersionConflict", err)
		}
		if err := s.DeleteComment(ctx, edited.ID, edited.Version); err != nil {
...
func runLinks(t *testing.T, newStore Factory) {
	t.Run("adds, lists and removes", func(t *testing.T) {
		...
		if err := s.RemoveLink(ctx, link); err != nil {
			t.Fatalf("RemoveLink: %v", err)
		}
		if err := s.RemoveLink(ctx, link); !errors.Is(err, tracker.ErrNotFound) {
